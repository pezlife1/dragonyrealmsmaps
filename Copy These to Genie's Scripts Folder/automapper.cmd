...
ABSOLUTE_TOP:
     ...
     var move_RETRY ^\.\.\.wait|^Sorry, you may only|^Sorry, system is slow|^You can't ride your.+(broom|carpet) in that direction|^You can't ride your.+(broom|carpet) in that direction|^The weight of all|lose your balance during the effort|^You are still stunned|^You're still recovering from your recent|^As you start to move\, a shooting pain|^You walk smack into the wall\.
...
actions:
     ...
     action (mapper) goto stowfootitem when at your feet, and do not wish to leave it behind\.|^You find yourself unable to sneak with items at your feet\.
...
move.real:
     ...
     if "$zoneid" = "69" && "$roomid" = "263" then
        {
            if !matchre("$roomobjs","gouged stone wall") then send turn basin
        }
     if "$zoneid" = "69" && "$roomid" = "264" then
        {
            if !matchre("$roomobjs","gouged stone wall") then send turn torch
        }
     send %movement
     goto return
move.power:
...
footwear.check:
     ...
     matchre wearing.skates ^some .* ice skates
     put inv feet
     matchwait 8
