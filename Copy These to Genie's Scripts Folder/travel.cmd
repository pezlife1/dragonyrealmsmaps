...
# USAGE - .travel <city> <room>  (room is optional!) (room can be either a number or name)
...
##########################################
##    RANKS TO USE SHARD WALLS          ##
    var shardwalls 300
##########################################
# NOW WITH THIEF GUILD ACCESS FOR SHARD/HAVEN!!! ####
# MAKE SURE YOU HAVE GLOBAL VARIABLES SET WITH YOUR
# GUILD PASSWORDS!  SHARD - shard.pw  HAVEN - haven.pw
# USAGE - .travel shard thief OR .travel haven thief
##########################################
...
###########################################
# CHANGELOG - Latest Update: 6/26/2019
# - Added travel to Thief guild in Riverhaven and Shard
# - Added option for %2 to move to a roomid or name when arriving at the location
...
action goto NOCOIN when You haven\'t got enough (.+) to pay for your trip\. Come back|You reach your funds\, but realize you\'re short\.|\"Hey\,\" he says\, \"You haven\'t got enough lirums to pay for your trip\.|You haven\'t got enough Lirums to pay for your trip\.
...
if matchre("(sha|shar|shard)","%destination") then 
     {
          var detour shard
          goto ILITHI
     }
...
CROSSING:
  ...
  if "$zoneid" = "67" && $Athletics.Ranks > %shardwalls then
            {
                gosub MOVE 134
                put climb embrasure
                wait
  ...
  pause 0.01
  if ("$zoneid" = "50") && matchre("(dirge|arthe|kaerna|viper|haven|theren|lang|rakash|muspari|zaulfung|cross|crossing)","%destination") && ($Athletics.Ranks > %segoltha) then gosub MOVE 8
  if ("$zoneid" = "50") && matchre("(knife|wolf|tiger|dokt)","%destination") && ($Athletics.Ranks > %segoltha) then gosub MOVE 36
  ...
  if "$zoneid" = "4a" && matchre("tiger","detour then goto arrived
  if "$zoneid" = "4a" then gosub MOVE 15
  if "$zoneid" = "4b" then gosub MOVE 1
  if "$zoneid" = "4" && matchre("(knife|wolf|tiger|dokt)","%detour") then 
            {
                if "%detour" = "wolf" then gosub MOVE 126
                if "%detour" = "knife" then gosub MOVE 459
                if "%detour" = "tiger" then gosub MOVE 87
                if "%detour" = "dokt" then gosub MOVE dokt
                goto ARRIVED
            }
  if "$zoneid" = "4" && $Athletics.Ranks > %segoltha && matchre("(leth|acen|taipa|ratha)","%detour") then
            {
                gosub MOVE 87
                gosub MOVE 73
                gosub MOVE 30
  ...
  if "$zoneid" = "1" && matchre("(wolf|knife|tiger|dokt)","%detour") then
            {
                gosub MOVE 172
                if "%detour" = "wolf" then gosub MOVE 126
                if "%detour" = "knife" then gosub MOVE 459
...
ILITHI:
  ...
  if "$zoneid" = "12a" then gosub MOVE 60
  if "$zoneid" = "4a" then gosub MOVE 15
  if "$zoneid" = "4" && $Athletics.Ranks >= %segoltha then 
            {
                gosub MOVE 87
                gosub MOVE 73
                gosub MOVE 30
...
ILITHI_2:
  ...
  if "$zoneid" = "69" && $Athletics.Ranks > %shardwalls && matchre("(shard|adan'f|corik)","%detour") then gosub move 592
            #{
            #    gosub MOVE 10
            #    put climb wall
            #    wait
  ...
  if "$zoneid" = "67" && $Athletics.Ranks > %shardwalls && matchre("(steel|ylono|fayrin)","%detour") then
            {
                gosub MOVE 134
                put climb embrasure
                wait
            }
  if "$zoneid" = "67" && $Athletics.Ranks > %shardwalls && matchre("(horse|spire|wyvern)","%detour") then
            {
                gosub MOVE 128
                put climb embrasure
                wait
  ...
  if "$zoneid" = "66" && $Athletics.Ranks > %shardwalls && matchre("(shard|adan'f|corik)","%detour") then gosub move 681
            #{
            #    gosub MOVE 70
            #    put climb wall
            #    wait
...
ARRIVED:
     if "%2" = "thief" then
        {
            if "$zoneid" = "30" then
                {
                    action goto foundleader when Lady Swan|Stolen Goods|Crow
                    gosub move gl thief coop
                    pause
                    gosub move gl thief half
                    pause
                    gosub move gl thief pawn
                    pause
                    gosub move gl thief silver
                    pause
                    gosub move gl thief cresc
                    pause
                    gosub move gl thief rook
                    pause
                    ECHO ===== DID NOT FIND THIEF GL CONTACT CHRIS FOR DEBUG =====
                }
            if "$zoneid" = "67" then
                {
                    gosub move 149
     ...
     foundleader:
    echo THANK YOU FOR USING THE TRAVEL SCRIPT BY:
    echo    ____ _                
    echo   / ____ | |           (_)    
    echo  | |          | |__   __ _ ___ 
    echo  | |          | '_   \|  _|   |/ __|
  ...
  echo IF YOU HAVE ANY FEATURE REQUESTS, DON'T HESITATE TO ASK!
  echo
...
JOINLOGIC:
  pause 0.1
  pause 0.1
  matchre ONJOINED ^\[Aboard the Dirigible\, Gondola\]|^An intricate network of silken rope|^\[Aboard the Balloon\, Gondola\]|^A veritable spiderweb of ropes secures|^Thick\, barnacle\-encrusted ropes secure the platform to the|\[Aboard the Mammoth\, Platform\]|\[The Bardess\' Fete\, Deck\]|^Silken rigging suspends the sweeping teak
  put look
  wait
  pause
  if matchre("$roomobjs","airship") then put join airship
  if matchre("$roomobjs","dirigible") then put join dirigible;join dirigible
  if matchre("$roomobjs","Gnomish balloon") then put join balloon;join balloon
...
ONJOINED:
  pause 0.1
  pause 0.1
  matchre OFFJOINED ^The grasses of this wide clearing|^From its northwest\-facing position|^The ironwood platform has withstood|^A rickety platform in the top of this huge\,|^Beyond the harbor\, spray is thrown|^Giant boulders are scattered|^Crudely assembled yet sturdy just the|\[Fang Cove\, Dock\]|\[Outside Muspar\'i\]|\[Northeast Wilds\, Grimsage Way\]|^The once pristine tower of the Warrior Mages
  matchwait
OFFJOINED:
  put look
  wait
